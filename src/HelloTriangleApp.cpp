...
void HelloTriangleApp::pick_physical_device()
{
    ...
    // The graphics card we select will be stored in a 'vk::PhysicalDevice' handle.
    // This object will be implicitly destroyed when the 'vk::Instance' is destroyed,
...
void HelloTriangleApp::create_logical_device()
{
    ...
    // The remainder of the information bears resemblance to vk::InstanceCreateInfo
    ...
    //       This means that the enabledLayerCount and ppEnabledLayerNames fields of vk::CreateInfo are ignored by
...
void HelloTriangleApp::create_swap_chain()
{
    ...
    createInfo.imageUsage = vk::ImageUsageFlagBits::eColorAttachment;// The kind of operations we'll use the swap chain images for
    ...
    // You'll almost always want to simply ignore the alpha channel, hence vk::CompositeAlphaFlagBitsKHR::eOpaque.
...
void HelloTriangleApp::create_render_pass()
{
    ...
    colorAttachment.initialLayout = vk::ImageLayout::eUndefined;  // WHat layout to have before the render pass begins
    colorAttachment.finalLayout = vk::ImageLayout::ePresentSrcKHR; // What layout to automatically transition to once the render pass finishes
...
void HelloTriangleApp::create_graphics_pipeline()
{
    ...
    vertShaderStageInfo.stage = vk::ShaderStageFlagBits::eVertex; // Which pipeline stage the shader will belong to
    ...
    rasterizer.polygonMode = vk::PolygonMode::eFill; // How are fragments generated? (Fill, Line, Point)
    rasterizer.lineWidth = 1.0f;
    rasterizer.cullMode = vk::CullModeFlagBits::eBack; // Type of face culling to use
    rasterizer.frontFace = vk::FrontFace::eClockwise; // Specify the vertex order for faces to be considered front-facing
...
bool HelloTriangleApp::is_device_suitable(vk::PhysicalDevice device)
{
    ...
    // be queried using PhysicalDevice.getProperties(deviceProperties)
//    vk::PhysicalDeviceProperties deviceProperties;
//    device.getProperties(deviceProperties);
    
    // The support for optional features like texture compression, 64bit floats and multi-viewport
    // rendering (useful for VR) can be queried using PhysicalDevice.getFeatures()
//    vk::PhysicalDeviceFeatures deviceFeatures;
...
vk::SurfaceFormatKHR HelloTriangleApp::choose_swap_surface_format(const std::vector<vk::SurfaceFormatKHR> &availableFormats)
{
...
vk::PresentModeKHR HelloTriangleApp::choose_swap_present_mode(const std::vector<vk::PresentModeKHR> &availablePresentModes)
{
